...
namespace WUP{
	...
	ReturnStatusAsync CCacheBuffer::Write(LPBYTE pBuf, INT cbBufferSize)
	{
	...
	ReturnStatusAsync CCacheBuffer::Peek(LPBYTE pBuf, INT cbBufferSize, INT &rcbPeek)
	{
		if (cbBufferSize == 0 || pBuf == NULL)
		{
			LOG(ERROR) << "读取长度或内容不合法";
	...
	ReturnStatusAsync CCacheBuffer::Read(LPBYTE pBuf, INT cbBufferSize, INT &rcbRead)
	{
		if (cbBufferSize == 0 || pBuf == NULL)
		{
			LOG(ERROR) << "读取长度或内容不合法";
