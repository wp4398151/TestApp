...
GraphicsSystem *g_GraphSys = NULL;
GraphicsSystem::GraphicsSystem(): curMatrix(0)
{
...
Shader* GraphicsSystem::CreateVertexShaderFromFile(CTSTR lpFileName)
{
    ...
    String fullPathFilenameHLSL;
    String fullPathFilenameCSO;

	if ((lpFileName[0] != '.' && lpFileName[0] != '/' && lpFileName[0] != '\\') && !(lpFileName[0] && lpFileName[1] == ':'))
	{
		fullPathFilenameHLSL << g_pObsAPI->GetAppPath() << L"\\" << lpFileName << L".vShader";
		fullPathFilenameCSO << g_pObsAPI->GetAppPath() << L"\\" << lpFileName << L"_V.cso";
	}
	else
	{
        fullPathFilenameHLSL << lpFileName<<L".vShader";
        fullPathFilenameCSO << lpFileName<<L"_V.cso";
	}

	if (!ShaderFile.Open(fullPathFilenameHLSL, XFILE_READ | XFILE_SHARED, XFILE_OPENEXISTING))
	{
		CrashError(TEXT("CreateVertexShaderFromFile: Couldn't open %s: %d"), fullPathFilenameHLSL, GetLastError());
	}

    String strShader;
    ShaderFile.ReadFileToString(strShader);
    if (strShader.IsEmpty())
		CrashError(TEXT("CreateVertexShaderFromFile: Couldn't read %s: %d"), fullPathFilenameHLSL, GetLastError());

	return CreateVertexShader(strShader, fullPathFilenameCSO);
}
...
Shader* GraphicsSystem::CreatePixelShaderFromFile(CTSTR lpFileName)
{
	...
	if ((lpFileName[0] != '.' && lpFileName[0] != '/' && lpFileName[0] != '\\') && !(lpFileName[0] && lpFileName[1] == ':'))
	{
		fullPathFilenameHLSL << g_pObsAPI->GetAppPath() << L"\\" << lpFileName << L".pShader";
		fullPathFilenameCSO << g_pObsAPI->GetAppPath() << L"\\" << lpFileName << L"_P.cso";
	}
	else
	{
		fullPathFilenameHLSL << lpFileName << L".pShader";
		fullPathFilenameCSO << lpFileName << L"_P.cso";
	}

    if (!ShaderFile.Open(fullPathFilenameHLSL, XFILE_READ | XFILE_SHARED, XFILE_OPENEXISTING))
		CrashError(TEXT("CreatePixelShaderFromFile: Couldn't open %s: %d"), fullPathFilenameHLSL, GetLastError());

    String strShader;
    ShaderFile.ReadFileToString(strShader);
    if (strShader.IsEmpty())
		CrashError(TEXT("CreatePixelShaderFromFile: Couldn't read %s: %d"), fullPathFilenameHLSL, GetLastError());

    return CreatePixelShader(strShader, fullPathFilenameCSO);
}
...
//        fullPathFilename << g_pObsAPI->GetAppPath() << L"\\" << fileName;
//    else
...
//
//            g_GraphSys->CreatePixelShaderBlob(c.shaderData, strShader.Array(), c.fileName.c_str());
//
...
{
    MatrixStack[curMatrix].Scale(scale.P2.x, scale.P2.y, 1.0f);
    ResetViewMatrix();
}
