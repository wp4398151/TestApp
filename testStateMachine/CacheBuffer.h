...
#define CACHEBLOCKSIZE 1024 
	...
	public:
		bool Init();
		INT GetSize();
		ReturnStatusAsync Read(LPBYTE pBuf, INT cbBufferSize, INT &rcbRead);
		ReturnStatusAsync Peek(LPBYTE pBuf, INT cbBufferSize, INT &rcbPeek);
		ReturnStatusAsync Write(LPBYTE pBuf, INT cbBufferSize);
		void Clear();
	...
	private:
		...
		typedef std::list<CacheBlock*>::iterator CacheBlockListItor;
