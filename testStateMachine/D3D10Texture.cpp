...
Texture* D3D10Texture::CreateFromSharedHandle(unsigned int width, unsigned int height, HANDLE handle)
{
    ...
    if(!handle)
    {
		...
		MessageBox(NULL, TEXT("游戏使用的显卡和当前选择的显卡不一致。请切换显卡！"), TEXT("提示"), MB_OK);
        return NULL;
    }
    ...
    newTex->m_format = GetGSFormatFromDXGIFormat(td.Format);;
    newTex->m_pResource = resource;
    newTex->m_pTexture = texVal;
    newTex->m_bDynamic = false;
...
Texture* D3D10Texture::CreateTexture(unsigned int width, unsigned int height, GSColorFormat colorFormat, void *lpData, BOOL bGenMipMaps, BOOL bStatic)
{
    ...
    D3D10Texture *newTex = new D3D10Texture;
    newTex->m_format = colorFormat;
    newTex->m_pResource = resource;
    newTex->m_pTexture = texVal;
    newTex->m_bDynamic = !bStatic;
    newTex->m_width = width;
    newTex->m_height = height;
...
Texture* D3D10Texture::CreateFromFile(CTSTR lpFile, BOOL bBuildMipMaps)
{
    ...
    D3D10Texture *newTex = new D3D10Texture;
    newTex->m_pResource = resource;
    newTex->m_pTexture = tex2D;
    newTex->m_width = ii.Width;
    newTex->m_height = ii.Height;

    switch(ii.Format)
    {
    case DXGI_FORMAT_R8_UNORM:
        newTex->m_format = GS_ALPHA;
        break;
    case DXGI_FORMAT_A8_UNORM:
        newTex->m_format = GS_GRAYSCALE;
        break;
    case DXGI_FORMAT_B8G8R8X8_UNORM:
        newTex->m_format = GS_BGR;
        break;
    case DXGI_FORMAT_B8G8R8A8_UNORM:
        newTex->m_format = GS_BGRA;
        break;
    case DXGI_FORMAT_R8G8B8A8_UNORM:
        newTex->m_format = GS_RGBA;
        break;
    case DXGI_FORMAT_R16G16B16A16_FLOAT:
        newTex->m_format = GS_RGBA16F;
        break;
    case DXGI_FORMAT_R32G32B32A32_FLOAT:
        newTex->m_format = GS_RGBA32F;
        break;
    case DXGI_FORMAT_BC1_UNORM:
        newTex->m_format = GS_DXT1;
        break;
    case DXGI_FORMAT_BC2_UNORM:
        newTex->m_format = GS_DXT3;
        break;
    case DXGI_FORMAT_BC3_UNORM:
        newTex->m_format = GS_DXT5;
        break;
        default:
        newTex->m_format = GS_UNKNOWNFORMAT;
    }
...
{
    return m_format == 1 || (m_format >= GS_RGBA && m_format <= GS_RGBA32F) || m_format == GS_DXT3 || m_format == GS_DXT5;
}

GSColorFormat D3D10Texture::GetFormat() const
{
    return (GSColorFormat)m_format;
}
...
bool D3D10Texture::GetDC(HDC &hDC)
{
    ...
    HRESULT err;
    if (FAILED(err = m_pTexture->QueryInterface(__uuidof(IDXGISurface1), (void**)&m_pSurface)))
    {
        AppWarning(TEXT("D3D10Texture::GetDC: could not query surface interface, result = %08lX"), err);
        return false;
    }

    if (FAILED(err = m_pSurface->GetDC(TRUE, &hDC)))
    {
...
void CopyPackedRGB(BYTE *lpDest, BYTE *lpSource, UINT nPixels)
{
    ...
    while(nDWords)
    {
        ...
        {
        case 0:
            *(lpDWDest++)  = *lpDWSrc         & 0xFFFFFF;
            *lpDWDest     = *(lpDWSrc++)>>24;
            break; //RBGR
        case 1:
            *(lpDWDest++) |= ((*lpDWSrc)<<8)  & 0xFFFF00;
            *lpDWDest     = *(lpDWSrc++)>>16;
            break; //GRBG
        case 2:
            *(lpDWDest++) |= ((*lpDWSrc)<<16) & 0xFFFF00;
            *(lpDWDest++) = *(lpDWSrc++)>>8;
            break; //BGRB
        }
...
void D3D10Texture::SetImage(void *lpData, GSImageFormat imageFormat, UINT pitch)
{
    ...
    {
    case GS_ALPHA:
        bMatchingFormat = (imageFormat == GS_IMAGEFORMAT_A8);
        pixelBytes = 1;
        break;
    case GS_GRAYSCALE:
        bMatchingFormat = (imageFormat == GS_IMAGEFORMAT_L8);
        pixelBytes = 1;
        break;
    case GS_RGB:
        bMatchingFormat = (imageFormat == GS_IMAGEFORMAT_RGB || imageFormat == GS_IMAGEFORMAT_RGBX);
        pixelBytes = 4;
        break;
    case GS_RGBA:
        bMatchingFormat = (imageFormat == GS_IMAGEFORMAT_RGBA);
        pixelBytes = 4;
        break;
    case GS_BGR:
        bMatchingFormat = (imageFormat == GS_IMAGEFORMAT_BGR || imageFormat == GS_IMAGEFORMAT_BGRX);
        pixelBytes = 4;
        break;
    case GS_BGRA:
        bMatchingFormat = (imageFormat == GS_IMAGEFORMAT_BGRA);
        pixelBytes = 4;
        break;
    case GS_RGBA16F:
        bMatchingFormat = (imageFormat == GS_IMAGEFORMAT_RGBA16F);
        pixelBytes = 8;
        break;
    case GS_RGBA32F:
        bMatchingFormat = (imageFormat == GS_IMAGEFORMAT_RGBA32F);
        pixelBytes = 16;
        break;
    }
    ...
    if (FAILED(err = m_pTexture->Map(0, D3D10_MAP_WRITE_DISCARD, 0, &map)))
    {
    ...
    //-------------------------------------------------------------------------
    if ((m_format == GS_RGB || m_format == GS_BGR) && (imageFormat == GS_IMAGEFORMAT_BGR || imageFormat == GS_IMAGEFORMAT_RGB))
    {
        if (pitch == (m_width*3) && map.RowPitch == (m_width*4))
            CopyPackedRGB((BYTE*)map.pData, (BYTE*)lpData, m_width*m_height);
        else
        {
            for (UINT y=0; y<m_height; y++)
            {
                ...
                CopyPackedRGB(curOutput, curInput, m_width);
            }
        }
    }
    //-------------------------------------------------------------------------
    else
    {
        if(pitch == map.RowPitch)
            mcpy(map.pData, lpData, pitch*m_height);
        else
        {
...
HANDLE D3D10Texture::GetSharedHandle()
{
	...
	if FAILED(err = m_pTexture->QueryInterface(IID_IDXGIResource, (void **)&pDXGIResource))
    {
