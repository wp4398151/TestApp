...
protected:
    ...
    inline HANDLE  GetViewProj() const
    {
        return hViewProj;
    }
...
{
    return g_GraphSys->CreateTexture(width, height, colorFormat, lpData, bGenMipMaps, bStatic);
}

inline Texture* CreateTextureFromFile(CTSTR lpFile, BOOL bBuildMipMaps)
{
    return g_GraphSys->CreateTextureFromFile(lpFile, bBuildMipMaps);
}

inline Texture* CreateRenderTarget(unsigned int width, unsigned int height, GSColorFormat colorFormat, BOOL bGenMipMaps)
{
    return g_GraphSys->CreateRenderTarget(width, height, colorFormat, bGenMipMaps);
}

inline Texture* CreateGDITexture(unsigned int width, unsigned int height)
{
    return g_GraphSys->CreateGDITexture(width, height);
}

inline Texture* CreateFromSharedHandle(unsigned int width, unsigned int height, HANDLE handle)
{
    return g_GraphSys->CreateTextureFromSharedHandle(width, height, handle);
}
inline Texture* CreateSharedTexture(unsigned int width, unsigned int height)
{
    return g_GraphSys->CreateSharedTexture(width, height);
}


inline SamplerState* CreateSamplerState(SamplerInfo &info)
{
    return g_GraphSys->CreateSamplerState(info);
}

inline Shader* CreateVertexShaderFromBlob(ShaderBlob const& blob, CTSTR lpShader, CTSTR lpFileName)
{
    return g_GraphSys->CreateVertexShaderFromBlob(blob, lpShader, lpFileName);
}
inline Shader* CreatePixelShaderFromBlob(ShaderBlob const& blob, CTSTR lpShader, CTSTR lpFileName)
{
    return g_GraphSys->CreatePixelShaderFromBlob(blob, lpShader, lpFileName);
}
inline Shader* CreateVertexShader(CTSTR lpShader, CTSTR lpFileName)
{
    return g_GraphSys->CreateVertexShader(lpShader, lpFileName);
}
inline Shader* CreatePixelShader(CTSTR lpShader, CTSTR lpFileName)
{
    return g_GraphSys->CreatePixelShader(lpShader, lpFileName);
}
inline Shader* CreateVertexShaderFromFile(CTSTR lpFileName)
{
    return g_GraphSys->CreateVertexShaderFromFile(lpFileName);
}
inline Shader* CreatePixelShaderFromFile(CTSTR lpFileName)
{
    return g_GraphSys->CreatePixelShaderFromFile(lpFileName);
}



inline VertexBuffer* CreateVertexBuffer(VBData *vbData, BOOL bStatic=1)
{
    return g_GraphSys->CreateVertexBuffer(vbData, bStatic);
}

inline void  LoadVertexBuffer(VertexBuffer* vb)
{
    g_GraphSys->LoadVertexBuffer(vb);
}
inline void  LoadTexture(Texture *texture, UINT idTexture=0)
{
    g_GraphSys->LoadTexture(texture, idTexture);
}
inline void  LoadSamplerState(SamplerState *sampler, UINT idSampler=0)
{
    g_GraphSys->LoadSamplerState(sampler, idSampler);
}
inline void  LoadVertexShader(Shader *vShader)
{
    g_GraphSys->LoadVertexShader(vShader);
}
inline void  LoadPixelShader(Shader *pShader)
{
    g_GraphSys->LoadPixelShader(pShader);
}


inline Shader* GetCurrentPixelShader()
{
    return g_GraphSys->GetCurrentPixelShader();
}
inline Shader* GetCurrentVertexShader()
{
    return g_GraphSys->GetCurrentVertexShader();
}

inline void  SetRenderTarget(Texture *texture)
{
    g_GraphSys->SetRenderTarget(texture);
}


inline void  Draw(GSDrawMode drawMode, DWORD StartVert=0, DWORD nVerts=0)
{
    g_GraphSys->Draw(drawMode, StartVert, nVerts);
}

inline void  EnableBlending(BOOL bEnabled)
{
    g_GraphSys->EnableBlending(bEnabled);
}
inline void  BlendFunction(GSBlendType srcFactor, GSBlendType destFactor, float fFactor=1.0f)
{
    g_GraphSys->BlendFunction(srcFactor, destFactor, fFactor);
}

inline void  ClearColorBuffer(DWORD color=0xFF000000)
{
    g_GraphSys->ClearColorBuffer(color);
}

inline void  StartVertexBuffer()
{
    g_GraphSys->StartVertexBuffer();
}
inline VertexBuffer* SaveVertexBuffer()
{
    return g_GraphSys->SaveVertexBuffer();
}
inline void  Vertex(float x, float y, float z=0)
{
    g_GraphSys->Vertex(x, y, z);
}
inline void  Vertex(const Vect &v)
{
    g_GraphSys->Vertex(v);
}
inline void  Vertex(const Vect2 &v2)
{
    g_GraphSys->Vertex(v2);
}
inline void  Normal(float x, float y, float z)
{
    g_GraphSys->Normal(x, y, z);
}
inline void  Normal(const Vect &v)
{
    g_GraphSys->Normal(v);
}
inline void  Color(DWORD dwRGBA)
{
    g_GraphSys->Color(dwRGBA);
}
inline void  Color(const Color4 &v)
{
    g_GraphSys->Color(v);
}
...
{
    Color4 rgba(R,G,B,A);
    g_GraphSys->Color(rgba);
}
inline void  TexCoord(float u, float v, int idTexture=0)
{
    g_GraphSys->TexCoord(u, v, idTexture);
}
inline void  TexCoord(const UVCoord &uv, int idTexture=0)
{
    g_GraphSys->TexCoord(uv, idTexture);
}

inline void  Ortho(float left, float right, float top, float bottom, float znear, float zfar)
{
    g_GraphSys->Ortho(left, right, top, bottom, znear, zfar);
}
inline void  Frustum(float left, float right, float top, float bottom, float znear, float zfar)
{
    g_GraphSys->Frustum(left, right, top, bottom, znear, zfar);
}

inline void  SetViewport(float x, float y, float width, float height)
{
    g_GraphSys->SetViewport(x, y, width, height);
}
inline void  SetScissorRect(XRect *pRect=NULL)
{
    g_GraphSys->SetScissorRect(pRect);
}

inline void DrawSprite(Texture *texture, DWORD color, float x, float y)
{
    if (!texture) return;
    g_GraphSys->DrawSprite(texture, color, x, y, (float)texture->Width(), (float)texture->Height());
}
inline void DrawSprite(Texture *texture, DWORD color, float x, float y, float x2, float y2)
{
    g_GraphSys->DrawSprite(texture, color, x, y, x2, y2);
}
inline void DrawSpriteEx(Texture *texture, DWORD color, float x, float y, float x2, float y2, float u, float v, float u2, float v2)
{
    g_GraphSys->DrawSpriteEx(texture, color, x, y, x2, y2, u, v, u2, v2);
}
inline void DrawBox(const Vect2 &upperLeft, const Vect2 &size)
{
    g_GraphSys->DrawBox(upperLeft, size);
}
